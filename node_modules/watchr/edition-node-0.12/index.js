...
/* :Array<any> */
{
  ...
  return _construct(Stalker, args);
}
/**
Stalker
...
function (_EventEmitter) {
  ...
  */
  ...
  /* :string */
    ...
    _this = _possibleConstructorReturn(this, _getPrototypeOf(Stalker).call(this)); // Ensure global watchers singleton

    if (Stalker.watchers == null) Stalker.watchers = {}; // Add our watcher to the singleton

    if (Stalker.watchers[path] == null) Stalker.watchers[path] = new Watcher(path);
    _this.watcher = Stalker.watchers[path]; // Add our stalker to the watcher

    if (_this.watcher.stalkers == null) _this.watcher.stalkers = [];

    _this.watcher.stalkers.push(_assertThisInitialized(_assertThisInitialized(_this))); // If the watcher closes, remove our stalker and the watcher from the singleton
  ...
  _createClass(Stalker, [{
    ...
    */
    ...
    /* :?string */
      ...
      // Remove our stalker
      this.remove(); // If it was the last stalker for the watcher, or if the path is deleted
    ...
    */
    ...
    /* :Array<any> */
    {
      ...
      (_this$watcher = this.watcher).setConfig.apply(_this$watcher, arguments);
    ...
    */
    ...
    /* :Array<any> */
    {
      ...
      (_this$watcher2 = this.watcher).watch.apply(_this$watcher2, arguments);
...
function (_EventEmitter2) {
  ...
  */
  ...
  /* :string */
    ...
    _this3 = _possibleConstructorReturn(this, _getPrototypeOf(Watcher).call(this)); // Initialise properties
    ...
    _this3.listenerTimeout = null; // Initialize our configurable properties
  ...
  */
  ...
  _createClass(Watcher, [{
    ...
    /* :WatcherOpts */
      ...
      // Apply
      extendr.extend(this.config, opts); // Stat
    ...
    */
    ...
    /* :Array<any> */
    {
      ...
      // Emit the log event
      this.emit.apply(this, ['log'].concat(args)); // Chain
    ...
    */
    ...
    key: "isIgnoredPath",
    value: function isIgnoredPath(path
    /* :string */
    ) {
      // Ignore?
      var ignore = ignorefs.isIgnoredPath(path, this.getIgnoredOptions()); // Return
    ...
    */
    ...
    /* :StatCallback */
      ...
      var method = this.config.followLinks ? 'stat' : 'lstat'; // Fetch
    ...
    */
    ...
    /* ::?:ErrorCallback */
      ...
      var previousStat = null; // Log

      this.log('debug', "watch via ".concat(method, " method fired on: ").concat(this.path)); // Delay the execution of the listener tasks, to once the change events have stopped firing
      ...
      }, config.catchupDelay || 0); // We are a subsequent listener, in which case, just listen to the first listener tasks
      ...
      var tasks = this.listenerTaskGroup = new TaskGroup("listener tasks for ".concat(this.path), {
        domain: false
      }).done(next);
      tasks.addTask('check if the file still exists', function (complete) {
        // Log
        _this5.log('debug', "watch evaluating on: ".concat(_this5.path, " [state: ").concat(_this5.state, "]")); // Check if this is still needed


        if (_this5.state !== 'active') {
          _this5.log('debug', "watch discarded on: ".concat(_this5.path));
        ...
        fsUtil.exists(_this5.path, function (exists) {
          ...
          previousStat = _this5.stat; // If the file still exists, then update the stat

          if (exists === false) {
            // Log
            _this5.log('debug', "watch emit delete: ".concat(_this5.path)); // Apply
            ...
            _this5.emit('change', 'delete', _this5.path, null, previousStat); // Clear the remaining tasks, as they are no longer needed
          ...
          _this5.getStat({
            ...
            // Check
            if (err) return complete(err); // Update

            currentStat = stat; // Complete
      ...
      tasks.addTask('check if the file has changed', function (complete) {
        ...
        if (currentStat.ino.toString() !== previousStat.ino.toString()) {
          _this5.log('debug', "watch found replaced: ".concat(_this5.path), currentStat, previousStat); // note this will close the entire tree of listeners and reinstate them
        ...
        if (currentStat.mtime.toString() !== previousStat.mtime.toString()) {
          _this5.log('debug', "watch found modification: ".concat(_this5.path), previousStat, currentStat);
      ...
      tasks.addGroup('check what has changed', function (addGroup, addTask, done) {
        ...
        if (currentStat.isDirectory() === false) {
          ...
          _this5.log('debug', "watch emit update: ".concat(_this5.path));
        ...
        fsUtil.readdir(_this5.path, function (err, newFileRelativePaths) {
          // Error?
          if (err) return done(err); // Log

          _this5.log('debug', "watch read dir: ".concat(_this5.path), newFileRelativePaths); // Find deleted files


          eachr(_this5.children, function (child, childFileRelativePath) {
            // Skip if the file still exists
            if (newFileRelativePaths.indexOf(childFileRelativePath) !== -1) return; // Fetch full path

            var childFileFullPath = pathUtil.join(_this5.path, childFileRelativePath); // Skip if ignored file

            if (_this5.isIgnoredPath(childFileFullPath)) {
              _this5.log('debug', "watch ignored delete: ".concat(childFileFullPath, " via: ").concat(_this5.path));

              return;
            } // Emit the event and note the change


            _this5.log('debug', "watch emit delete: ".concat(childFileFullPath, " via: ").concat(_this5.path));
          ...
          eachr(newFileRelativePaths, function (childFileRelativePath) {
            ...
            if (_this5.children[childFileRelativePath] != null) return; // Fetch full path

            var childFileFullPath = pathUtil.join(_this5.path, childFileRelativePath); // Skip if ignored file

            if (_this5.isIgnoredPath(childFileFullPath)) {
              _this5.log('debug', "watch ignored create: ".concat(childFileFullPath, " via: ").concat(_this5.path));
            ...
            addTask('watch the new child', function (complete) {
              _this5.log('debug', "watch determined create: ".concat(childFileFullPath, " via: ").concat(_this5.path));
    ...
    */
    ...
    value: function close() {
      var reason
      /* :string */
      = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 'unknown';
      // Nothing to do? Already closed?
      if (this.state !== 'active') return this; // Close

      this.log('debug', "close: ".concat(this.path)); // Close our children
      ...
      this.log('debug', "watch closed because ".concat(reason, " on ").concat(this.path));
      this.emit('close', reason); // Chain
    ...
    */
    ...
    key: "watchChild",
    value: function watchChild(opts
    /* :WatchChildOpts */
    , next
    /* :ErrorCallback */
    )
    ...
    {
      // Prepare
      var watchr = this; // Create the child

      var child = create(opts.fullPath); // Apply the child

      this.children[opts.relativePath] = child; // Add the extra listaeners
      ...
      child.watch(next); // Return the child
    ...
    */
    ...
    key: "watchChildren",
    value: function watchChildren(opts
    /* :Object */
    , _next
    /* :ErrorCallback */
    ) {
      // Prepare
      var watchr = this; // Check stat
      ...
      if (this.stat.isDirectory()) {
        scandir({
          ...
          next: function next(err, list) {
            ...
            var tasks = new TaskGroup("scandir tasks for ".concat(path), {
              domain: false,
              concurrency: 0
            }).done(_next);
            Object.keys(list).forEach(function (relativePath) {
              tasks.addTask(function (complete) {
                var fullPath = pathUtil.join(path, relativePath); // Check we are still relevant

                if (watchr.state !== 'active') return complete(); // Watch this child
    ...
    */
    ...
    key: "watchMethod",
    value: function watchMethod(method
    /* :MethodEnum */
    , next
    /* :ErrorCallback */
    )
    ...
    */
    ...
    /* :ErrorCallback */
      ...
      var _opts$preferredMethod = opts.preferredMethods,
          preferredMethods = _opts$preferredMethod === void 0 ? this.config.preferredMethods : _opts$preferredMethod;
      opts.errors = errors;
      opts.preferredMethods = preferredMethods; // Attempt the watch methods
    ...
    */
    ...
    /* :Array<any> */
    {
      ...
      this.log('debug', "watch init: ".concat(this.path)); // Fetch the stat then try again

      this.getStat({}, function (err) {
        if (err) return next(err); // Watch ourself

        _this8.watchSelf({}, function (err) {
          if (err) return next(err); // Watch the children

          _this8.watchChildren({}, function (err) {
            if (err) {
              _this8.close('child failure');

              _this8.log('debug', "watch failed on [".concat(_this8.path, "] with ").concat(errorToString(err)));
            } else {
              _this8.log('debug', "watch success on [".concat(_this8.path, "]"));
            }
  ...
  return Watcher;
}(EventEmitter); // Now let's provide node.js with our public API
